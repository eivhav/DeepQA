...
class LanguageModel:
    ...
    def get_similarity(self):
        ...
        if similarity == 'cosine_tf':
            return lambda x: dot(x[0], x[1]) / K.maximum(K.sqrt(dot(x[0], x[0]) * dot(x[1], x[1])), K.epsilon())
        elif similarity == 'cosine_theano':
            return lambda x: K.dot(x[0], K.transpose(x[1])) / (x[0].norm(2) * x[1].norm(2))
    ...
    def compile(self, optimizer, **kwargs):
        ...
        gamma_log = -10.0
        hinge_loss = lambda x: K.relu(self.config['margin'] - x[0] + x[1])
        hinge_loss_log = lambda x: K.log(1.0 + K.exp(gamma_log * K.relu(self.config['margin'] - x[0] + x[1])))
...
class EmbeddingModel(LanguageModel):
    def build(self):
        ...
        maxpool.supports_masking = False
        enc = Dense(128, activation='tanh')
        question_pool = enc(maxpool(question_embedding))
        answer_pool = enc(maxpool(answer_embedding))
